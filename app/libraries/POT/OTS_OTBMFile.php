 ...
 */
	...
	//Sleavely: multidimensional array in order Z, X, Y
	public $tiles = array();
 ...
 */
						...
						if( $tileNode->getType() == self::OTBM_NODE_TILE)
						{
							...
							while($item){
								if($item->getType() == self::OTBM_NODE_ITEM){
									if(!isset($tile['items'])) $tile['items'] = array();
	...
	//Sleavely: parse an OTBM item node recursively
	private function parseItem($node){
		...
		while($child){
			if($child->getType() == self::OTBM_NODE_ITEM){
				if(!isset($item['items'])) $item['items'] = array();
				$item['items'][] = $this->parseItem($child);
			}
			$child = $child->getNext();
		}
	...
	//Sleavely: add a tile to $this->tiles
 ...
 */
	...
	//Sleavely: returns array of towns
